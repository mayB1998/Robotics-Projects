...
class Visual_Odometry():
    def __init__(self, data_dir, descriptor="ORB", num_features=1000, matcher="FLANN", show_matches=True):
    ...
    def get_pose(self, img1_l, img2_l, img1_r, img2_r):
        """
        ...
        tp1_l, tp1_r, tp2_l, tp2_r = self.calculate_right_qs(tp1_l, tp2_l, self.disparity_old, disparity)
        self.disparity_old = disparity
...
def main(render=False, pass_gt=False):
    data_dir = "/home/NEU_Courses/EECE5554/Final_Project/Dataset/KITTI_sequences/test"
    ...
    for i in tqdm.tqdm(range(len(image_paths_r)), unit="pose"):
        if i == 0:
            cur_pose = gt_poses[0]
            img0_l = cv2.imread(image_paths_l[i], cv2.IMREAD_GRAYSCALE)
            img0_r = cv2.imread(image_paths_r[i], cv2.IMREAD_GRAYSCALE)
            vo.disparity_old = np.divide(vo.disparity.compute(img0_l, img0_r).astype(np.float32), 16)
        else:
            img1_l = cv2.imread(image_paths_l[i], cv2.IMREAD_GRAYSCALE)
            img1_r = cv2.imread(image_paths_r[i], cv2.IMREAD_GRAYSCALE)

            transformation = vo.get_pose(img0_l, img1_l, img0_r, img1_r)
